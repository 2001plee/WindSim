...
class Vertex { //used in load obj to get verticies
public:
...
//Unused in our main render loop
/*
...
//potential other hash //unused as seems make code run super slow, but it is what paper said.
...
//makes the hash. Same as described general online and in paper
...
//find neighboring particles with spatial hashing
...
//old neighbor not optimized with spatial hash
/*
...
*/
//old pressure and density. Incorrect derivative i think just keeping it for later
/*
...
void compute_pressure(std::vector<Arrow>& particles, float k, float p0) { //p0 seems to be p(rho) from slides this means it should be set to 1.3f
    for (int i = 0; i < particles.size(); ++i) { //go through all the particles.
        ...
        //ƒÏi (difference) is density of particle i
        //idk why 7
        //-1 set to 0 i think when other part (ƒÏi/ƒÏ0)^7 is 1 cuz equal?
...
//derivative of current smoothing kernel
...
glm::vec3 compute_pressure_force(float radius, int current_index_i, const std::vector<int>& neighbor_indices, const std::vector<Arrow>& arrows) { //const keep it same
    ...
    for (int neighbor_index : neighbor_indices) {//just for loop take index and iterates through neighbor_indiices
        ...
        float denominator_squared_j = density_j * density_j; // ƒÏj^2
...
//compute all forces needed in SPH all in one function and reason why all function for SPH are commented out as done all at once.
...
//update particle position based off velocity will take in a vector of vec3 forces for resultant_force from compute_viscosity_and_pressure_force_other
...
//old reflect function before parallel threads
/*
...
//modified Moller Trumbore that takes in particle position and next position in step then check for 
//intersection with triangle, face, and if so return intersectionPoint
...
//struct to return in parallel thread from checking intersection for reflect ot work
struct intersectchunk {
    bool hit;
...
//old intersesction kept for testing without index search and parallel thread to prove optimization
/*
...
//using this generate cube of arrow when testing not used anymore
/*
...
//randomize generate better verison of old one that deals with random gen one in the demo used (The actual generate)
//takes in vector of arrow, ylayers, a layer being the y value of the layer where x and z are the constraints set to be 20-40 for z 
//and currently takes mid point around X at 0 then expands x constraints by arrowCubeSize. Then it takes # arrow you want per layer
...
int main() {
    ...
    */
    ...
    glBindVertexArray(0);
        ...
        //loads the obj and reads verticies and normals 
        ...
        //
    int numArrows = 0;
    float sph_radius = 3.0f; //2.6 is with 5000 arrow per layer and ylayer is 30 layers
    float k_constant = 5.0f;
    float air_density = 1.3f; 
    int numArrowsPerLayer = 1000; //# of arrows per layer
    ...
    std::vector<float> yLayers = { 200, 202.5, 205, 207.5, 210, 212.5, 215, 217.5, 220, 222.5, 225, 227.5, 230, 232.5, 235, 237.5, 240, 242.5, 245,247.5}; //4 layers
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        //Start Imgui
            ...
            if (ImGui::SliderInt("Arrows in each Layer", &numArrowsPerLayer, 0, 50000)) { //unused cuz for debugging and turned off since camera uses mouse
        ...
        */
        ...
        //commented out Very first version of our code with a removed find neighbor search above for 
        // loop tried throwing openmp and stuff. its way slower keep this code for old showcase in presentation show difference
        /*
...
#pragma omp critical
        ...
        */
        ...
        //our render code with parallel processing (MAIN RENDER LOOP STUFF FOR SPH and intersection)
        std::vector<std::vector<int>> neighbor_hood; //holds index to the arrows
        ...
        int count = 0;
        //generating color by faces, so triangles since triangulation from tiny obj loader
